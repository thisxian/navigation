export default function jsonp(url: any, config: any) {
    var data = config.data || [];
    var paraArr = [], paraString = '';//get请求的参数。
    var urlArr;
    var callbackName: any;//每个回调函数一个名字。按时间戳。
    var script: any, head: any;//要生成script标签。head标签。
    var supportLoad;//是否支持 onload。是针对IE的兼容处理。
    var onEvent: any;//onload或onreadystatechange事件。
    ...
    paraArr.push(`${config.srcbName || 'callback'}=` + callbackName);
    ...
    script[onEvent] = function () {
        ...
        script = script[onEvent] = script.onerror = (window as any)[callbackName] = null;

    }

    script.onerror = function () {
        if ((window as any)[callbackName] == null) {
    ...
    if (timeout != 0) {
        setTimeout(function () {
            if (script && script.loaded == false) {
                (window as any)[callbackName] = null;//超时，且未加载结束，注销函数
                script.onerror();
            }
        }, timeout);
    }
